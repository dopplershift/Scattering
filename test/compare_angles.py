...
def plot_csecs(l, scatterers):
    for s in scatterers:
        plt.subplot(2,2,1)
        plot_csec(s, l, 10.0*np.log10(s.get_reflectivity_factor(mp)) + ref_adjust, 'Ref. Factor')
        plt.subplot(2,2,2)
        plot_csec(s, l, s.get_attenuation(mp) * consts.kilo * db_factor, 'Attenuation')
        plt.subplot(2,2,3)
        plot_csec(s, l, 10.0*np.log10(s.get_reflectivity(mp, polar='h')/s.get_reflectivity(mp, polar='v')), 'ZDR')
        plt.subplot(2,2,4)
        plot_csec(s, l, (s.get_attenuation(mp, polar='h') - s.get_attenuation(mp, polar='v'))  * consts.kilo * db_factor, 'Diff. Atten')
